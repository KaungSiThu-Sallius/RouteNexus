...
REGION_VALUE_MULTIPLIERS = {
    "Strait of Malacca": 1.05,
    "Red Sea": 1.22,
    "Suez Canal": 0.98,
    "Panama Canal": 0.94,
    "Taiwan Strait": 1.34,
    "South China Sea": 1.12,
    "East China Sea": 1.08,
    "Sea of Japan": 1.01,
    "Andaman Sea": 0.89,
    "Gulf of Thailand": 0.93,
    "English Channel": 1.11,
    "North Atlantic": 1.18,
}
CARGO_TERM_ALIASES = {
    "semiconductor": ["semiconductor", "chip", "chips", "wafer", "wafers"],
    "crude oil": ["crude", "oil", "petroleum"],
    "consumer electronics": ["electronics", "electronic"],
    "medical supplies": ["medical", "medicine", "pharma", "pharmaceutical"],
    "automotive components": ["automotive", "auto", "vehicle", "car"],
    "industrial machinery": ["machinery", "machine", "industrial"],
    "battery cells": ["battery", "batteries", "cell", "cells"],
    "containerized retail goods": ["retail", "container", "goods"],
    "lng equipment": ["lng", "lpg", "gas"],
    "telecom hardware": ["telecom", "network", "hardware"],
}
...
def _extract_cargo_filters(message_text: str) -> set[str]:
    ...
    matched_filters = set()
    for cargo_name, aliases in CARGO_TERM_ALIASES.items():
        if any(alias in lowered for alias in aliases):
            matched_filters.add(cargo_name)
...
def check_inventory_exposure(message_text: str, region: str) -> str:
    ...
    try:
        ...
        critical_vessels = 0
        vessel_names = []
        region_multiplier = REGION_VALUE_MULTIPLIERS.get(region, 1.0)
